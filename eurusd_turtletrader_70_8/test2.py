...
TWS_TIMEZONE = pytz.timezone('Asia/Shanghai')
...
###############################################################################
    def run(self):
        ...
        #self.log('Waiting until the market opens ({}:{})...'.format(
        #    open_hr, open_min))
        #while True:
        ...
        # Get rid of this if the above code is reimplemented ^^^^^^
        now = datetime.datetime.now(tz=pytz.timezone('US/Eastern'))
...
###############################################################################  --Modified for BBs
    def long_entry_signal(self, instrument):
        ...
        # Entry logic
        if RSI < 30:
            self.log('{} LONG ENTRY SIGNAL: price={}, RSI={}'.format(
                    instrument.symbol, price, RSI))
            return True
        return False
...
############################################################################### --Modified for BBs
    def long_exit_signal(self, instrument):
        ...
        # Exit logic
        if RSI > 50:
            self.log('{} LONG EXIT SIGNAL: price={}, RSI={}'.format(
                    instrument.symbol, price, RSI))
            return True
        return False
...
############################################################################### -- Modified for BBS
    def short_entry_signal(self, instrument):
        ...
        # Entry logic
        if RSI > 70:
            self.log('{} SHORT ENTRY SIGNAL: price={},RSI={}'.format(
                    instrument.symbol, price, RSI))
            return True
        return False
...
############################################################################### -- Modified for BBS
    def short_exit_signal(self, instrument):
        ...
        # Exit logic
        if RSI < 50:
            self.log('{} SHORT BB EXIT SIGNAL: price={}, RSI={}'.format(
                    instrument.symbol, price, RSI))
            return True
        return False
...
# Get desired qty - no function for this currently
        #desired_qty = -QTY
        #desired_qty = -(self.get_FR_qty(instrument))
        # This next one was originally used:
        # desired_qty = -(self.get_fxd_qty(instrument))
        # Get order quantity
        # this one was used:
        # order_qty = desired_qty-qty
...
###############################################################################
    def get_quantity(self, instrument):
        ...
        elif str(type(instrument))[-7:-2] == 'Forex':
...
###############################################################################
    def add_instrument(self, instrument_type, ticker, last_trade_date=None,
                       ...
                       future_exchange='GLOBEX', forex_exchange='IDEALPRO', 
                       pair='EURUSD', symbol='EUR', quote='USD'):
        ...
        if instrument_type == 'Stock':
            ...
            instrument = Forex(ticker, exchange=forex_exchange, 
                symbol=symbol, currency=quote)
        else:
            raise ValueError(
...
###############################################################################
    def add_bar(self, bar):
        ...
        if bar[-3:] == 'min' or bar[-4:] == 'mins':
            ...
            bar_minutes = 60*int(bar.split(' ')[0])
            self.bars_minutes.append(bar_minutes)
...
###############################################################################
    def get_RSI(self, df):
        ...
        df['RSI'] = abs(df['avg_gain']/df['avg_loss'])
        df['RSI'] = 100.0-(100.0/(1+df['RSI']))
        return df
...
if __name__ == '__main__':
    ...
    algo.add_instrument('Forex', ticker='EURUSD', symbol='EUR', quote='USD')
    algo.add_instrument('Forex', ticker='GBPJPY', symbol='GBP', quote='JPY')
    #algo.add_instrument('Option', 'SPY', '20190920', 300, 'C')
    #algo.add_instrument('Future', 'ES', '20190920')
    PNL = algo.create_PNL(['EURUSD'])
    PNL = algo.create_PNL(['GBPJPY'])

    algo.create_fxd_dict(['EURUSD'])
    algo.create_fxd_dict(['GBPJPY'])
    ...
    # Add HH to indicators
    algo.add_HL(length=10)
